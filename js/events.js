...
function addBlds(n){
  ...
  for(let a=0;a<200&&c<n;a++){
    ...
    if(G.map[y][x].type==='grass'&&!G.map[y][x].bld&&!G.map[y][x].annex&&(typeof hasPowerPlant!=='function'||!hasPowerPlant(x,y))&&(typeof dcIndexAt!=='function'||dcIndexAt(x,y)<0)&&nb(x,y).some(([ax,ay])=>ax>=0&&ax<MAP&&ay>=0&&ay<MAP&&G.map[ay][ax].type==='road')){
